...
#ifdef ZCL_READ
static uint8_t Generic_ProcessInReadRspCmd( zclIncoming_t *pInMsg );
#endif
#ifdef ZCL_WRITE
static uint8_t Generic_ProcessInWriteRspCmd( zclIncoming_t *pInMsg );
#endif
static uint8_t Generic_ProcessInDefaultRspCmd( zclIncoming_t *pInMsg );
#ifdef ZCL_DISCOVER
static uint8_t Generic_ProcessInDiscCmdsRspCmd( zclIncoming_t *pInMsg );
static uint8_t Generic_ProcessInDiscAttrsRspCmd( zclIncoming_t *pInMsg );
static uint8_t Generic_ProcessInDiscAttrsExtRspCmd( zclIncoming_t *pInMsg );
#endif
...
static void Generic_Init( void )
{
  ...
  Generic_DstAddr.addrMode = (afAddrMode_t)AddrNotPresent;
  ...
  zclGenericAppEpDesc.simpleDesc = &Generic_SimpleDesc;
...
static void Generic_initializeClocks(void)
{
    ...
    Generic_processEndDeviceRejoinTimeoutCallback,
    GENERIC_END_DEVICE_REJOIN_DELAY,
    0, false, 0);
#endif
...
#if ZG_BUILD_ENDDEVICE_TYPE
/*******************************************************************************
 * @fn      Generic_processEndDeviceRejoinTimeoutCallback
 *
...
static void Generic_processEndDeviceRejoinTimeoutCallback(UArg a0)
{
    ...
    appServiceTaskEvents |= GENERIC_END_DEVICE_REJOIN_EVT;
...
static void Generic_process_loop( void )
{
    ...
    for(;;)
    {
        ...
        if(Semaphore_pend(appSemHandle, BIOS_WAIT_FOREVER ))
        {
          ...
          if(appServiceTaskEvents & GENERIC_KEY_EVT)
          {
              // Process Key Presses
...
#if ZG_BUILD_ENDDEVICE_TYPE
        if ( appServiceTaskEvents & GENERIC_END_DEVICE_REJOIN_EVT )
        {
        ...
        /* GENERIC_TODO: handle app events here */

        if ( appServiceTaskEvents & GENERIC_EVT_1 )
        {

          appServiceTaskEvents &= ~GENERIC_EVT_1;
        }

        /*
        if ( appServiceTaskEvents & GENERIC_EVT_2 )
        {

          appServiceTaskEvents &= ~GENERIC_EVT_2;
        }

        if ( appServiceTaskEvents & GENERIC_EVT_3 )
        {

          appServiceTaskEvents &= ~GENERIC_EVT_3;
        }
        */
      }
  }
    ...
    case BDB_COMMISSIONING_PARENT_LOST:
      ...
      else
      {
        //Parent not found, attempt to rejoin again after a fixed delay
          Timer_setTimeout( EndDeviceRejoinClkHandle, GENERIC_END_DEVICE_REJOIN_DELAY );
...
static uint8_t Generic_ProcessIncomingMsg( zclIncoming_t *pInMsg )
{
  ...
  switch ( pInMsg->hdr.commandID )
  {
    ...
    case ZCL_CMD_READ_RSP:
      Generic_ProcessInReadRspCmd( pInMsg );
      handled = TRUE;
      break;
#endif
#ifdef ZCL_WRITE
    case ZCL_CMD_WRITE_RSP:
      Generic_ProcessInWriteRspCmd( pInMsg );
      handled = TRUE;
      break;
#endif
    ...
    case ZCL_CMD_DEFAULT_RSP:
      Generic_ProcessInDefaultRspCmd( pInMsg );
      handled = TRUE;
      break;
#ifdef ZCL_DISCOVER
    case ZCL_CMD_DISCOVER_CMDS_RECEIVED_RSP:
      Generic_ProcessInDiscCmdsRspCmd( pInMsg );
      handled = TRUE;
      break;
    ...
    case ZCL_CMD_DISCOVER_ATTRS_RSP:
      Generic_ProcessInDiscAttrsRspCmd( pInMsg );
      handled = TRUE;
      break;

    case ZCL_CMD_DISCOVER_ATTRS_EXT_RSP:
      Generic_ProcessInDiscAttrsExtRspCmd( pInMsg );
      handled = TRUE;
      break;
#endif
    default:
...
#ifdef ZCL_READ
/*********************************************************************
 * @fn      Generic_ProcessInReadRspCmd
 *
...
#ifdef ZCL_WRITE
/*********************************************************************
 * @fn      Generic_ProcessInWriteRspCmd
 *
...
/*********************************************************************
 * @fn      Generic_ProcessInDefaultRspCmd
 *
...
#ifdef ZCL_DISCOVER
/*********************************************************************
 * @fn      Generic_ProcessInDiscCmdsRspCmd
 *
...
/*********************************************************************
 * @fn      Generic_ProcessInDiscAttrsRspCmd
 *
...
/*********************************************************************
 * @fn      Generic_ProcessInDiscAttrsExtRspCmd
 *
